...
def docker_run(run_args, print_output=True):
  ...
  stdout = None if print_output else open(os.devnull, 'w')
  try:
...
def coverage(args):
  ...
  if (not args.no_corpus_download and not args.corpus_dir
      and not args.project.is_external) and not download_corpora(args):
    return False
  ...
  run_args.extend(['-v', f'{args.project.out}:/out', '-t', BASE_RUNNER_IMAGE])
